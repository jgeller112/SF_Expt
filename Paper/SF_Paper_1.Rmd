...
Another startegy that may impart a desirable difficulty involves changing the physical characteritics of stimuli to make it harder to read or more disfluent.  Whereas making stimuli easier to proccess or more fluent produces overconfidence (predicted memory > actual memory; @Rhodes2008, @Rhodes2009), disflueny (i.e., the subjective experience of effort during processing) serves to better calibrate learners and produces better memory. In a now infamous paper, @Diemand-Yauman2011 demonstrated this in the labortarto and a classroom by placing to-be studied materials in atypical fonts (e.g., Comic Sans). Placing materails in atypical fonts resulted in better memory than if the material was in a common font. This finding has been found with other perceptual manipulations such as masking [presenting a row of hashmarks before the presentation of a stimulus;  @Mulligan1996], inversion [@Sungkhasettee2011], blurring [@Rosner2015], and handwriting [@Geller2018]. The predominate theoritical explanaton for this is that the experience of disfluency serves as a metacognitive (subjective) cue, and this engenders deeper, more semantic, processing of material [but see @Geller2018 for an alterantive account]. The benefit associated with processing difficulty brought forth by changing perceptual characteristics of stimuli is called the disfluency effect. 

Given the desribale effects on memory and the relative ease of implementation (i.e., individuals just need to place material in an atypical or hard to read font), it is clear why perceptual disfluency is such an appealing strategy. However, there have been several experiments that failed to find memorial benefits for perceptually disfluent materials [e.g., @Magreehan2016; @Rhodes2008, @Rhodes2009; @Rummer2016; @Yue2013], casting doubt upon the robustness of the disfluency effect. Corroborating this, A recent meta-analysis by @Xie2018 with 25 studies and 3,135 participants found a small, non-significant, effect of perceptual disfluency on recall and (*d* = -0.01) and transfer (*d* = 0.03). Despite having no mnemonic effect, perceptual disfluency produced longer reading times (*d* = 0.52) and lower judgments of learning (i.e., metamemory judgements that assess future memory) (*d* =  -0.043).

Although evidence for perceptual disflueny is weak, it important to point out that there are important boundary conditions of the disfluency effect (see Geller & Still, 2018). One important boundary condition to consider is the level of disfluency used. Not all perceptual disfluency manipulations are created equal. Using handwritten cursive that was either easy to read or hard to read, they observed an inverted u-shaped pattern--easy to read and hard to read cursive were better remembered than print, but easy to read cursive was better remembered than hard to read cursive. This pattern suggests that manipulations that are disfluent can produce better memory, but they need to be optimally disfluent, or just right to do so.

Recently, a team of psychologists, graphic designers, and marketers claimed to have created a font that provides an optimal level of perceptual disflueny to act as a desirable difficulty--called Sans Forgetica. Sans Forgetica is a variation of a sans seirf font that has intermittent gaps in letters that are back slanted (see fig. 1). It is the fonts characteristics that are thought to draw attention to the stimuli and elicit deeper processing. The positive effect of Sans Forgetica font on memory has stirred up a lot of attention and press. In 2019, it won the GoodDesign, Best in Class award (Good Desigm, 2019). Sans Forgetica has alos been covered by major news sources like the Washington Post [@Telford2018] and National Public Radio (NPR; https://www.npr.org/2018/10/06/655121384/sans-forgetica-a-font-to-remember). Sans Forgetica font is even available on multiple operating platforms.
...
Initial evidence for the Sans Forgetica font comes from 2 unpublished studies by the Sans Forgetica team [@Earp2018; also see @Eskenazi2020]. To find the optimal font, the Sans Forgetica team first conducted a cued recall experiment (*N*=96) werein participants read 20 highly associated word pairs (e.g., girl - guy) each for 100 ms in a moderate disfluent font (e.g., Sans Forgetica), an extremely disfluent font, a slighly disfluent font, and a normal, fluent, font. The found that pairs in Sans Forgetica font were recalled slighly better (69%) than the normal font (68%). In a follow-up online experiment (*N* = 303), participants were presented passages (250 words in total) where one of the paragraphs was presented in Sans Forgetica. Each participant saw five different texts in total. For each text participants were asked one question about the part written in SF and another question about the part written in standard Arial. Placing text passages in Sans Forgetica font resulted in better memeory (57%) than if matearis were presented in normal font (50%).

To examine these claims, @Taylor examined the Sans Forgetica effect acorss several experiments with over 882 participants. In none of their experiments did Sans Forgeticae enhance memory. In fact, Sans Forgetica seemed to harm memory (Expeirment 2). 

The current set of experiments comprised an attempt to conceptually replicate and extend the work by @Taylor and the Sans Forgetica team to examine whether Sans Forgetica font has a desirable effect on memory. As the @Taylor study is the only published replication attempt, we thought it pertinent to follow up on their claims. The question of whether Sans Forgetica produces a mnnmenomic benefit has clear practical implications. In the educational domain, it would be relatively quick and easy to place materials in Sans Forgetica font. However, in order for Sans Forgetica font to be useful, it is important to note and understand both its successes and failures. Given the equivocal findings in the literature, the current research aims to identify important moderating or boundary conditions of the Sans Fogetica effect (Oppenheimer & Alter, 2014). 

In three high-powered experiments, we examined some potential boundary conditions of the Sans Forgetica effect. Experiment 1, we examined cue strength by looking at weakly related cue-target recall. In Experiment 2, we examined design type by examining prose passage recall using a between-subjects manipulation. In Experiment 3, type of test is examined by using a yes/no recognition test. In addition, we aimed to compared the Sans forgetica effect with other notable learning technqies--generation (Experiment 1) and pre-highlighting (Experiment 2). Comparing Sans Forgetica to other study techniques allows us to examine the mechanisms underlying the effect, if any. 
...
sfgen=read_csv(here::here("Expt1_data", "sfgenerate_final.csv"))
