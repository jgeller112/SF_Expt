...
```{r, fig.cap= "Mean proportions of “old” responses for Experiment 3. The fixed effect mean (black dot) and 95% CIs derived from the glmer model" }
...
oldnewglme=glmer(sayold~old.new*condition+(1|Participant.Private.ID)+ (1|Stims), data=ex3, family=binomial(link="probit"))
...
means$old.new<-factor(means$old.new, levels=c("old", "new"))

means$condition<-factor(means$condition, levels=c("SF", "normal"))

p1<- ggplot(means, aes(condition, Probability, color=condition))+ facet_grid(~old.new)+
  geom_line(data=means,aes(y=Probability, group=1), size=1)+ 
  geom_pointrange(data=means, aes(y=Probability, ymin=CI_low, ymax=CI_high), size=1, color="black")+ 
  theme_bw(base_size=14)+
